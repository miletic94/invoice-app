...
export default function SummaryBanner(props) {
  ...
  return (
    <div className="summaryBannerWrapper">
      <Link to={{ pathname: `invoice/${props.id}` }} key={props.id}>
        <div className="summaryBanner">
          ...
          <div className="clientName fontBg">{props.clientName}</div>
          <h3 className="totalCost">
            <span className="currency">£</span>
            <span className="amount">
              {props.total.toFixed(2).toLocaleString()}
            </span>
          </h3>
          <InfoBadge status={props.status} />
          <svg
            className="arrowRightSummaryBanner"
            width="7"
            height="10"
